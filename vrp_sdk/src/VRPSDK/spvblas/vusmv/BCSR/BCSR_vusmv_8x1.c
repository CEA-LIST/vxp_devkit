...
static inline void _BCSR_dvusmv_8x1(args_t args)
{/* !!!! c * (8*1) */
    ...
    for (int brow = 0; brow < nbr; brow++) {
        ...
        for (int i = brow_start; i < brow_end; i++) {
            ...
            pld(P9 , bval_ptr+7*c*sizeof(double), 0, EFP0);      // first bval7
            ...
            for (int j = 0; j < c-1; j++) {
                ...
                pmul(P17, P9 , P23, EC0);                            // bval7 * x0
                pld(P9 , bval_ptr+7*c*sizeof(double), 0, EFP0);      // next bval7
                ...
                padd(P31, P31, P17, EC0);                            // acc7 += bval7*x0
            }
        ...
        pmul(P31, P31, P0, EC0);                                 // acc6 *= alpha
        ple(P9 , y_ptr, 7, EVP1);                                // y7
        ...
        pse(P17, y_ptr, 7, EVP1);                                // mem(y7) = y7

        y_ptr += 8*args.y_bytes;
    }
}

static inline void _BCSR_dvusmv_trans_8x1(args_t args)
{/* !!!! c * (8*1) */
    int c = args.a->col_block_size;
    ...
    for (int brow = 0; brow < nbr; brow++) {
        ...
        pmul(P31, P31, P0 , EC0);                                    // x7 *= alpha

        x_ptr += 8*args.x_bytes;
        for (int i = brow_start; i < brow_end; i++) {
            ...
            pld(P8, bval_ptr+7*c*sizeof(double), 0, EFP0);       // bval7
            ...
            for (int j = 0; j < c-1; j++) {
                ...
                padd(P23, P23, P16, EC0);                            // y0 += bval7*x7
...
void BCSR_dvusmv_8x1(args_t args)
{
    if (args.trans == 'N')
        _BCSR_dvusmv_8x1(args);
    else
        _BCSR_dvusmv_trans_8x1(args);
}
