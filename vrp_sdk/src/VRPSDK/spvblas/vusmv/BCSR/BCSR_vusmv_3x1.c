...
static inline void _BCSR_dvusmv_3x1(args_t args)
{/* !!!! c * (3*1) */
    ...
    for (int brow = 0; brow < nbr; brow++) {
        ...
        pcvt_d_p(P29, 0);                                        // acc0 = 0
        pcvt_d_p(P30, 0);                                        // acc1 = 0
        pcvt_d_p(P31, 0);                                        // acc2 = 0

        for (int i = brow_start; i < brow_end; i++) {
            x_ptr = (uintptr_t) args.x + bind[i] * args.x_bytes;

            ple(P28, x_ptr, 0, EVP0);                            // x0
            ...
            for (int j = 0; j < c-1; j++) {
                ...
                pmul(P5 , P2 , P28, EC0);                            // bval0 * x0
                pld(P2 , bval_ptr+0*c*sizeof(double), 0, EFP0);      // next bval0
                pmul(P6 , P3 , P28, EC0);                            // bval1 * x0
                pld(P3 , bval_ptr+1*c*sizeof(double), 0, EFP0);      // next bval1
                pmul(P7 , P4 , P28, EC0);                            // bval2 * x0
                pld(P4 , bval_ptr+2*c*sizeof(double), 0, EFP0);      // next bval2

                ple(P28, x_ptr, 0, EVP0);                            // next x0

                bval_ptr += sizeof(double);
                x_ptr += args.x_bytes;

                padd(P29, P29, P5 , EC0);                            // acc0 += bval0*x0
                padd(P30, P30, P6 , EC0);                            // acc1 += bval1*x0
                padd(P31, P31, P7 , EC0);                            // acc2 += bval2*x0
            }
            ...
            bval_ptr += (3*c-c)*sizeof(double);
        }

        pmul(P29, P29, P0, EC0);                                 // acc0 *= alpha
        ple(P2 , y_ptr, 0, EVP1);                                // y0
        pmul(P30, P30, P0, EC0);                                 // acc1 *= alpha
        ple(P3 , y_ptr, 1, EVP1);                                // y1
        pmul(P31, P31, P0, EC0);                                 // acc2 *= alpha
        ple(P4 , y_ptr, 2, EVP1);                                // y2

        pmul(P5 , P2 , P1, EC0);                                 // y0 *= beta
        pmul(P6 , P3 , P1, EC0);                                 // y1 *= beta
        pmul(P7 , P4 , P1, EC0);                                 // y2 *= beta

        padd(P5 , P5 , P29, EC0);                                // y0 += acc0
        pcvt_d_p(P29, 0);                                        // acc0 = 0
        padd(P6 , P6 , P30, EC0);                                // y1 += acc1
        pcvt_d_p(P30, 0);                                        // acc1 = 0
        padd(P7 , P7 , P31, EC0);                                // y2 += acc2
        pcvt_d_p(P31, 0);                                        // acc2 = 0

        pse(P5 , y_ptr, 0, EVP1);                                // mem(y0) = y0
        pse(P6 , y_ptr, 1, EVP1);                                // mem(y1) = y1
        pse(P7 , y_ptr, 2, EVP1);                                // mem(y2) = y2

        y_ptr += 3*args.y_bytes;
    }
}

static inline void _BCSR_dvusmv_trans_3x1(args_t args)
{/* !!!! c * (3*1) */
    int c = args.a->col_block_size;
...
void BCSR_dvusmv_3x1(args_t args)
{
    if (args.trans == 'N')
        _BCSR_dvusmv_3x1(args);
    else
        _BCSR_dvusmv_trans_3x1(args);
}
